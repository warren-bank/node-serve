...
const {process_middleware_rules} = require('./test-path')
    ...
    if (paths.redirect.proxy && (req_url.substring(0, 4).toLowerCase() === 'http')) {
      ...
      try {
        ...
        if (proxyMiddleware.length) {
          ...
          const mime = data.response.headers['content-type'] ? data.response.headers['content-type'].toLowerCase() : ''
          if (mime) {

            const is_mime = {
              html: mime_types_regex.html && mime_types_regex.html.test(mime),
              json: mime_types_regex.json && mime_types_regex.json.test(mime),
              js:   mime_types_regex.js   && mime_types_regex.js.test(  mime),
              text: mime_types_regex.text && mime_types_regex.text.test(mime)
            }

            if (is_mime.html || is_mime.json || is_mime.js || is_mime.text) {
