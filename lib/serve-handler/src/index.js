...
const getHandlers = dependency_injection => Object.assign({
...
const serve_handler = async (request, response, config = {}, dependency_injection = {}) => {
  const req_method_whitelist = ["GET","HEAD","OPTIONS"]
  const req_method           = (request.method || 'GET').toUpperCase()
  ...
  if (
    (req_method_whitelist.indexOf(req_method) === -1) &&
    (
      (!paths.redirect) ||
      (paths.redirect.target.substring(0,4).toLowerCase() !== 'http')
    )
  ) {
    // method not allowed
    response.setHeader('allow', req_method_whitelist.join(', '))
    return process_error(getError(405))
  }
  ...
  if (!request.headers.range && headers.etag && (headers.etag === request.headers['if-none-match'])) {
    return sendDataPayload(req_method, response, null, 304)
  }
  ...
  if (req_method === 'HEAD') {
    ...
    sendDataPayload(req_method, response, null, response.statusCode || 200, headers)
  }
  else {
    let stream = null
