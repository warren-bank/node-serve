...
const sourceMatches = (source, requestPath, allowSegments, p2r_options={}) => {
  const keys  = []
  let results = null

  if (allowSegments) {
    const expression = p2r.pathToRegexp(source, keys, p2r_options)
  ...
  if (results || minimatch(requestPath, source)) {
    return {
      keys,
      results
    }
...
const toTarget = (source, destination, previousPath, encode) => {
  const p2r_options = {delimiter: '/', validate: false, encode: encode ? encodeURLPath : undefined}
  const matches     = sourceMatches(source, previousPath, true, p2r_options)
...
const applyRewrites = (requestPath, rewrites = [], repetitive) => {
  ...
  for (let index = 0; index < rewritesCopy.length; index++) {
    ...
    const target = toTarget(source, destination, requestPath, /* encode= */ false)
...
const shouldRedirect = (decodedPath, {redirects = [], trailingSlash}, cleanUrl) => {
  ...
  if (cleanUrl && matchHTML.test(decodedPath)) {
    ...
    return {
      target: encodeURLPath(ensureSlashStart(decodedPath)),
  ...
  if (slashing) {
    ...
    if (target) {
      return {
        target: encodeURLPath(ensureSlashStart(target)),
        statusCode: defaultType,
        preserveQuery: true,
        preserveHash:  true
      }
  ...
  for (let index = 0; index < redirects.length; index++) {
    ...
    const target = toTarget(source, destination, decodedPath, /* encode= */ true)
...
const getPaths = async (request, config, handlers) => {
  ...
  try {
    ...
    paths.virtual.rewrittenPath = applyRewrites(paths.virtual.relativePath, config.rewrites)
  }
  catch (err) {
...
const serve_handler = async (request, response, config = {}, methods = {}) => {
  ...
  if (paths.redirect) {
    ...
    response.writeHead(paths.redirect.statusCode, {
      Location: paths.redirect.target + req_search + req_hash
    })
