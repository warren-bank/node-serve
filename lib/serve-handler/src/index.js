...
const sourceMatches = (source, requestPath, allowSegments) => {
  ...
  if (allowSegments) {
    ...
    const expression = p2r.pathToRegexp(normalized, keys)
...
const toTarget = (source, destination, previousPath, encode) => {
  ...
  const options         = {validate: false, encode: encode ? encodeURLPath : undefined}
  const toPath          = p2r.compile(normalizedDest, options)
  const props           = {}
