    ...
    if (paths.redirect.proxy && (req_url.substring(0, 4).toLowerCase() === 'http')) {
      ...
      try {
        ...
        if (proxyMiddleware.length) {
          ...
          const mime_html_regex = new RegExp('^(?:text/html|application/xhtml|text/xml|application/xml).*$')

          if (mime && mime_html_regex.test(mime)) {
            // apply middleware to html/xml formatted text response
            ...
            try {
              ...
              for (let i=0; i < proxyMiddleware.length; i++) {
                proxyMiddleware[i].middleware($)

                if (proxyMiddleware[i].terminal)
                  break
              }
