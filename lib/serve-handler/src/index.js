  ...
  for (let index = 0; index < files.length; index++) {
    ...
    try {
      ...
      if (file_paths.real.isSymLink) {
        ...
        // prevent display of .symlink file extension
        if (details.base.lastIndexOf('.symlink') === details.base.length - 8) {
          details.base = details.base.substring(0, details.base.length - 8)
        }
...
const processSymLinks = async (paths, handlers) => {
  ...
  for (; current_dir < parts.length; current_dir++) {
    ...
    else if (stats.isFile() && (path.extname(absolutePath) === '.symlink')) {
      paths.real.isSymLink = true
      absolutePath = fs.readFileSync(absolutePath, {encoding: 'utf8'}).trim().split("\n")[0].trim()
    }
  }
...
const getPaths = async (request, config, handlers) => {
  ...
  try {
    const parsed_url = url_parse(request.url)
      ...
      if ((req_method === 'POST') && (request.readableLength > 0)) {
        if (request.headers['content-type'].toLowerCase().startsWith('multipart/form-data')) {
          for (const header_name in request.headers) {
            subprocess.stdin.write(`${header_name}: ${request.headers[header_name]}\n`)
          }
