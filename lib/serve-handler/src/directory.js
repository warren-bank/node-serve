const {encodeHTML, encodeURLPath} = require('./encoders')
  ...
  <head>
    ...
    <style>
      ...
      body > main > ul#files > li > a.folder::before {
        content: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 347.479 347.479' style='enable-background:new 0 0 347.479 347.479' xml:space='preserve'><path style='fill:%23e0b03b' d='M292.251 79.766H103.644v-8.544c0-5.974-4.888-10.862-10.862-10.862H30.414c-5.975 0-10.862 4.888-10.862 10.862v8.544h-3.258C7.332 79.766 0 87.098 0 96.059v174.766c0 8.961 7.332 16.293 16.293 16.293h275.958c8.961 0 16.293-7.332 16.293-16.293V96.059c.001-8.961-7.331-16.293-16.293-16.293z'/><path style='fill:%23fff' d='M23.243 95.385h262.059v176.113H23.243z'/><path style='fill:%23ffc843' d='M312.426 271.293c-2.135 8.704-11.213 15.825-20.175 15.825H16.293c-8.961 0-14.547-7.121-12.412-15.825l34.598-141.05c2.135-8.704 11.213-15.825 20.175-15.825h275.958c8.961 0 14.547 7.121 12.412 15.825l-34.598 141.05z'/></svg>");
      }
      ...
      body > main > ul#files > li > div.file-stats > span {
        display: inline-block;
      }
      body > main > ul#files > li > div.file-stats > span.size {
        font-weight: bold;
      }
      body > main > ul#files > li > div.file-stats > span.size::after {
        display: inline-block;
        margin: 0 0.5em;
        content: '\\2022';
        color: #333;
...
const directoryTemplate = spec => {
  ...
  if (Array.isArray(paths) && paths.length) {
    for (let path of paths) {
      html += `<a href="/${encodeURLPath(path.url)}">${encodeHTML(path.name)}</a>`
    }
  }
  ...
  if (Array.isArray(files) && files.length) {
    for (let file of files) {
      html += '<li>'
      html +=   `<a href="${encodeURLPath(file.relative)}" title="${encodeHTML(file.title)}" class="${encodeHTML(file.type)} ${encodeHTML(file.ext)}">${encodeHTML(file.base)}</a>`

      if ((file.stats instanceof Object) && file.stats.mtime) {
        html += '<div class="file-stats">'

        if (file.stats.size)
          html += `<span class="size">${encodeHTML(file.stats.size)}</span>`

        html +=   `<span class="mtime">${encodeHTML(file.stats.mtime)}</span>`
        html += '</div>'
      }
