...
const text_rewrite_path = (request_path, {source, destination, decode}, reencode) => {
  try {
...
const rewrite_path = (encoded_path, rule, reencode) => {
  ...
  if (rule.engine) {
    switch(rule.engine.toLowerCase()) {
      case 'route':
...
const process_redirect_rules = (encoded_path, rules, allow_recursion, default_type=301) => {
  ...
  result = new_path
    ? {target: new_path, statusCode: (rule.type || default_type), preserveQuery: !!rule.preserveQuery, preserveHash: !!rule.preserveHash}
    : null
