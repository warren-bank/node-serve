// Native
const normalize_path      = require('path').posix.normalize
...
const p2r_process_source = (request_path, source, p2r_options={}) => {
  const keys    = []
...
const p2r_rewrite_path = (request_path, {source, destination, decode}, reencode) => {
  try {
    ...
    const matches     = p2r_process_source(request_path, source, p2r_options)
...
const glob_rewrite_path = (request_path, {source, destination, decode}, reencode) => {
  try {
    if (!minimatch(request_path, source, minimatch_options))
      throw 'no match'
...
const rewrite_path = (encoded_path, rule, reencode) => {
  ...
  const request_path = rule.decode ? normalize_path(decodeURIComponent(encoded_path)) : encoded_path
  let new_path = null
...
module.exports = {process_rewrite_rules, process_redirect_rules, rewrite_path, p2r_process_source}
